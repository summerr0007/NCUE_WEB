...
//撈出當前訂單的所有商品項目
$sql = "select file_name, name, price, quantity, pid from order_form, commodity where 
...
if($result = mysqli_query($link,$sql))
{
    ...
    while($row = mysqli_fetch_assoc($result))
    {
        ...
        $rows.="<tr><td><img src='images/".$row['file_name']."' height='100' alt='no img'/></td>
                ...
                <td class='text-right'>$".$row['price']."</td>
                <td>
                    <div class='input-group col-lg-5 offset-lg-7'>
            ...
            <div class="col mb-2">
                <div class="row">
                    <div class="col-sm-12  col-md-6">
                        <button class="btn btn-block btn-light" onClick="redir('index','',0)">繼續購物</button>
                    </div>
                    <div class="col-sm-12 col-md-6 text-right">
                        <button class="btn btn-lg btn-block btn-success text-uppercase" <?php if($rows=="") echo "disabled='disabled'" ;?> onClick="checkout(<?php echo $today; ?>,<?php echo $now_bill; ?>)">送出訂單</button>
                    </div>
                </div>
            </div>
        </div>
